...
RUN         sed -i 's/dl-cdn.alpinelinux.org/mirrors.ustc.edu.cn/g' /etc/apk/repositories && \
            apk add --no-cache --update libgcc libstdc++ ca-certificates libcrypto1.1 libssl1.1 libgomp expat git
...
RUN \
        DIR=/tmp/aom && AOM_VERSION=v3.1.1 && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        wget -O x265.tar.bz2 https://bitbucket.org/multicoreware/x265_git/get/${X265_VERSION}.tar.bz2 && \
        tar xf x265.tar.bz2 && \
...
COPY ./freetype-${FREETYPE_VERSION}.tar.gz /tmp/workdir/freetype-${FREETYPE_VERSION}.tar.gz
RUN  \
        DIR=/tmp/freetype && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        cp /tmp/workdir/freetype-${FREETYPE_VERSION}.tar.gz ./freetype-${FREETYPE_VERSION}.tar.gz && \
        # curl -sLO https://mirror.yongbok.net/nongnu/freetype/freetype-${FREETYPE_VERSION}.tar.gz && \
...
RUN  \
        ...
        make install && \
        rm -rf ${DIR}
COPY ./kvazaar-${KVAZAAR_VERSION}.tar.gz /tmp/workdir/kvazaar-${KVAZAAR_VERSION}.tar.gz
## kvazaar https://github.com/ultravideo/kvazaar
RUN \
        DIR=/tmp/kvazaar && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        mv /tmp/workdir/kvazaar-${KVAZAAR_VERSION}.tar.gz ./kvazaar-${KVAZAAR_VERSION}.tar.gz && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://xorg.freedesktop.org/archive/individual/proto/xcb-proto-1.14.1.tar.gz && \
        tar -zx --strip-components=1 -f xcb-proto-1.14.1.tar.gz && \
...
RUN \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://xcb.freedesktop.org/dist/libxcb-1.14.tar.gz && \
        tar -zx --strip-components=1 -f libxcb-1.14.tar.gz && \
        ACLOCAL_PATH="${PREFIX}/share/aclocal" ./autogen.sh && \
        ./configure --prefix="${PREFIX}" --disable-static --enable-shared && \
        make && \
        make install && \
...
RUN \
        DIR=/tmp/libzmq && LIBZMQ_VERSION=4.3.2 && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
...
COPY ./aom.tar.gz /tmp/workdir/aom.tar.gz
RUN \
        DIR=/tmp/aom && mkdir -p ${DIR} && \
        cd ${DIR} && \
        mv /tmp/workdir/aom.tar.gz ./aom.tar.gz && \
        tar -zx  -f aom.tar.gz && \
        mkdir -p ./aom_build && \
        cd ./aom_build && \
        cmake -DCMAKE_INSTALL_PREFIX="${PREFIX}" -DBUILD_SHARED_LIBS=1 .. && \
